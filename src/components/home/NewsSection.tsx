    ...
    <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm overflow-hidden">
      <div className="p-6">
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          {news.map((item) => (
            <article key={item._id || item.id} className="py-6 first:pt-0 last:pb-0 flex flex-col md:flex-row items-start">
              {item.imageUrl && (
                <img
                  src={item.imageUrl}
                  alt={item.title}
                  className="w-full md:w-1/3 lg:w-1/4 h-auto md:h-32 object-cover rounded-lg mr-4 mb-4 md:mb-0"
                />
              )}
